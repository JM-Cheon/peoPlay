...
<jsp:include page="../admin/common/adminMenu.jsp"/>
			...
			<div class="p-board-panel">
				...
				<section class="p-center-list">
							...
							<tbody>
								<c:forEach items="${ list }" var="QNA">
									<tr>
								        ...
								        <c:if test="${ QNA.disclosureStatus eq 'N'}" >
								            <c:out value="${QNA.inquiryTitles}"/>
								        </c:if>
							           </td>
							        ...
							        <c:if test="${QNA.disclosureStatus eq 'Y'}" >공개</c:if></td>
							            <td class="textQna"><c:if test="${QNA.inquiryYn eq 'N'}" >
							            <c:choose>
							                <c:when test="${QNA.writer.userRole eq 'ADMIN' }">
							                  <c:out value="${ QNA.inquiryYn }"/>
							                </c:when>
							                <c:otherwise>미답변</c:otherwise>
							            </c:choose>
							        </c:if>
							        <c:if test="${QNA.inquiryYn eq 'Y'}" >답변완료</c:if></td>
									</tr>
								</c:forEach>
								...
								<c:forEach items="${ searchList }" var="QNA">
							        ...
							        <c:if test="${QNA.disclosureStatus eq 'Y'}" >공개</c:if></td>
							            <td><c:if test="${QNA.inquiryYn eq 'N'}" >
							            <c:choose>
							                <c:when test="${QNA.writer.userRole eq 'ADMIN' }">
							                  <c:out value="${ QNA.inquiryYn }"/>
							                </c:when>
					...
					<!--검색-->
					<form id="searchForm"  action="${ pageContext.servletContext.contextPath }/QNAadmin/search"
...
const link = "${ pageContext.servletContext.contextPath }/QNAadmin/qnaList";
const searchLink = "${ pageContext.servletContext.contextPath }/QNAadmin/search";
